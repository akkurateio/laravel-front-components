            ...
            @if(agent()->isDesktop())
                ...
                @foreach ($elements as $element)
                    ...
                    @if (is_string($element))
                        <li class="page-item mr-1 mr-sm-2 disabled" aria-disabled="true">
                            <span
                                    class="page-link rounded-full text-xs-2xs text-lg-md font-bold px-3 py-2 px-md-4 py-md-3">{{ $element }}</span>
                        </li>
                    @endif
                        ...
                        @foreach ($element as $page => $url)
                            @if ($page == $paginator->currentPage())
                                <li class="page-item mr-1 mr-sm-2 active" aria-current="page">
                                    <span
                                            class="page-link rounded-full text-xs-2xs text-lg-md font-bold px-3 py-2 px-md-4 py-md-3">{{ $page }}</span>
                                </li>
                            @else
                                <li class="page-item mr-1 mr-sm-2">
                                    <a class="page-link rounded-full text-xs-2xs text-lg-md font-bold px-3 py-2 px-md-4 py-md-3"
